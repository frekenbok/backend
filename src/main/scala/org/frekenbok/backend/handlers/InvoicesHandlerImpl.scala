...
class InvoicesHandlerImpl(dao: InvoicesDao)(implicit ec: ExecutionContext) extends InvoicesHandler {
  ...
  def createInvoice(
    respond: InvoicesResource.createInvoiceResponse.type
  )(body: Invoice): Future[InvoicesResource.createInvoiceResponse] = {
    dao.add(body).map { _ =>
  ...
  def getInvoices(
    respond: InvoicesResource.getInvoicesResponse.type
  )(before: Option[Instant] = None, limit: Option[Int] = Option(20)): Future[InvoicesResource.getInvoicesResponse] = {
  ...
  def getSingeInvoice(
    respond: InvoicesResource.getSingeInvoiceResponse.type
  )(invoiceId: UUID): Future[InvoicesResource.getSingeInvoiceResponse] = {
